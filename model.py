...
class PCAE(nn.Module):
    ...
    def forward(self, x, device, mode='train'):
        ...
        # scalar, [128, 24, 144], [128, 24, 6], [128, 24, 1]
...
class OCAE(nn.Module):
    ...
    def forward(self, inp, x_m, d_m, device, mode='train'):
        ...
        # [128, 24, 1, 3, 3], [128, 24, 16], [128, 24, 1, 1, 1]
        ov_k, c_k, a_k = self.relu(object_parts[:, :, :9]).view(*object_parts.size()[:2], 1, 3, 3), self.relu(
            object_parts[:, :, 9:25]), self.sigmoid(object_parts[:, :, -1] + noise_1).view(*object_parts.size()[:2], 1,
                                                                                           1, 1)
        temp_a = []
        temp_lambda = []
        ...
        # scalar, [128, 24, 1], [128, 24, 24], [128, 24, 24]
...
class SCAE_LOSS(nn.Module):
    ...
    def forward(self, output_scae, b_c, k_c):
        # scalar, scalar, [128, 24, 1], [128, 24, 24], [128, 24, 24]
